...
class Profile(contextlib.ContextDecorator):
    ...
    def time(self):
        if self.cuda:
            # torch.cuda.synchronize(self.device) 用于确保在计时之前和之后，所有 CUDA 操作都已经完成，从而保证计时的准确性
    ...
    for k in "train", "val", "test":
        if data.get(k):  # prepend path
            if isinstance(data[k], str):
                x = (path / data[k]).resolve() # resolve用于获取文件路径的绝对路径
...
def non_max_suppression(
    ...
    xc = prediction[..., 4] > conf_thres  # candidates # prediction[..., 4] 将提取置信度值
